...
def main():
    ...
    conf = SparkConf().setAppName(app_name).set("spark.hadoop.yarn.resourcemanager.address", "192.168.0.104:8032")
    ...
    def get_hdfs_filepath(file_name):
        my_hdfs = os.getcwd()
        return os.path.join(my_hdfs,'data/' + file_name)
    ...
    perc = df1.filter(df1.lang_num > 1).filter(df1.user_rating == max_rating).count() * 100 / df1.filter(df1.user_rating == max_rating).count()
    ...
    dfp = pd.DataFrame(zip([25, 50, 75, 100],data), columns = ['percentile','avg_lang_num']).set_index('percentile')
    LOGGER.info("\n\n7.\tHow does app details contribute to user ratings\tDone!\n\n{}".format(dfp.head(10))+"\n")


    # 8 compare the statistics of different app groups/genres
    dfp = df1.groupBy("prime_genre").agg(F.avg(df1.lang_num),F.avg(df1.screenshots_num),F.avg(df1.rating_count_tot),F.avg(df1.MB)).toPandas()
    ...
    data = [q1.first()[0], q2.first()[0], q3.first()[0], q4.first()[0]]
    dfp = pd.DataFrame(zip([25, 50, 75, 100],data), columns = ['rating_percentile','avg_desc_len']).set_index('rating_percentile')
